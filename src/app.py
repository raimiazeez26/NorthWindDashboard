...
def top_employees(df):
    ...
    ds = dd[['employeeName', 'SalesAmount(USD)', 'EmployeeCity', 'EmpReportsTo']].sort_values("SalesAmount(USD)", axis=0, ascending=False)
    ds = ds.rename(columns = {'employeeName': 'Name', 'SalesAmount(USD)' : 'Revenue', 'EmployeeCity':'City', 'EmpReportsTo': 'Superior'})
...
def indicator_curr(name, value, prev_val):
    ...
    fig = go.Figure(go.Indicator(mode="number+delta",
                                 ...
                                 domain={'x': [0, 1], 'y': [0, 1]}))
    fig.update_traces(number_font_size=36, title_font_size=18, title_font_family='Roboto',
...
app = Dash(__name__, external_stylesheets=[dbc.themes.MINTY],
                 meta_tags=[{'name': 'viewport',
                       'content': 'width=device-width, initial-scale=1.0'}]) #dbc.themes.ZEPHYR]
server = app.server
...
app.layout = \
    dbc.Container([
        ...
        dbc.Row([
            #logo
            dbc.Col([
                html.Img(src=img_png, className='text-center border', 
...
def data_store(value):
    ...
    if value == None:
        ...
        prev_data = pd.read_csv('northwind.csv', index_col =[0])
...
def update_indicators(data, prev_data):
    ...
    if len(prev_data) == 0:
        ...
        #shipping costs
        fig_ship = dcc.Graph(figure=indicator_curr('Total Cost of Shipping', freight(data), None), style=indicator_style, 
                             config={'displayModeBar': False})
        ...
        #sales employees
        fig_outst = dcc.Graph(figure=indicator('Outstanding Orders', oustanding_orders(data), None), style=indicator_style, 
                            config={'displayModeBar': False}) 
        
        #shipping days
        fig_shippingdays = dcc.Graph(figure=indicator_days('Avg Days before Shipping', avg_days_shipping(data), None), style=indicator_style, 
                                     config={'displayModeBar': False})
        
    else:
        #revenue
        fig_rev = dcc.Graph(figure=indicator_curr('Total Revenue', revenue(data), revenue(prev_data)), style=indicator_style, 
                            config={'displayModeBar': False})
        
        #orders
        fig_ord = dcc.Graph(figure=indicator('Total no Of Order', total_orders(data), total_orders(prev_data)), style=indicator_style, 
                  config={'displayModeBar': False})
        
        #shipping costs
        fig_ship = dcc.Graph(figure=indicator_curr('Total Cost of Shipping', freight(data), freight(prev_data)), style=indicator_style, 
                             config={'displayModeBar': False})
        ...
        #shipping days
        fig_shippingdays = dcc.Graph(figure=indicator_days('Avg Days before Shipping', avg_days_shipping(data), avg_days_shipping(prev_data)), style=indicator_style, 
                                     config={'displayModeBar': False})
